...
async function seedUser () {
  try {
    ...
    userToBeInserted = await userDbService.findOne({ 'username':'Meta36' });
    if (!userToBeInserted) {  
      userToBeInserted = {
        'password':'ROSiVXDDtUXO51m',
        'username':'Meta36',
        'isDeleted':false,
        'isActive':true,
        'userType':authConstant.USER_TYPES.User
      };
      await userDbService.create(userToBeInserted);
    } else {
      userToBeInserted = {
        'password':'ROSiVXDDtUXO51m',
        'username':'Meta36',
      ...
      await userDbService.updateByQuery({ 'username':'Meta36' }, userToBeInserted);
    }
    userToBeInserted = await userDbService.findOne({ 'username':'Damaris38' });
    if (!userToBeInserted) {  
      userToBeInserted = {
        'password':'3WlUo4cX1360gVo',
        'username':'Damaris38',
        'isDeleted':false,
        'isActive':true,
      ...
      userToBeInserted.password = await bcrypt.hash(userToBeInserted.password, 8);
      await userDbService.updateByQuery({ 'username':'Damaris38' }, userToBeInserted);
    }
...
async function seedUserRole (){
  try {
    ...
    const insertedUsers = await userDbService.findMany({ username: { '$in': userRoles.map(userRole => userRole.username) } });
    let user = {};
    const userRolesArr = [];
