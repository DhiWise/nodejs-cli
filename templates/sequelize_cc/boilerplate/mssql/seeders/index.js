...
async function seedUserRole (){
  try {
    let user = await userDbService.findOne({
      'email':'Jodie_Metz62@hotmail.com',
      'isActive':true,
      'isDeleted':false
    });
...
async function seedUser () {
  try {
    ...
    if (!user || !user.isPasswordMatch('98t8MgUEp_E6xxi')) {
      let user = {
        'password':'98t8MgUEp_E6xxi',
        'email':'Jodie_Metz62@hotmail.com',
    ...
    if (!admin || !admin.isPasswordMatch('4nTx96HPTq9OPH_')) {
      let admin = {
        'password':'4nTx96HPTq9OPH_',
        'email':'Obie_Rowe20@gmail.com',
