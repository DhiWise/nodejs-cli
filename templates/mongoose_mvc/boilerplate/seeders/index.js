...
async function seedUser () {
  try {
    ...
    let user = await dbService.findOneAndUpdateDocument(User, { 'username':'Loyce_Leuschke' }, userToBeInserted,  {
      upsert: true,
      new: true
    });
    userToBeInserted = {
    ...
    userToBeInserted.password = await  bcrypt.hash(userToBeInserted.password, 8);
    let admin = await dbService.findOneAndUpdateDocument(User, { 'username':'Keegan_Daugherty' }, userToBeInserted,  {
      upsert: true,
      new: true
    });
