...
async function seedUserRole (){
  try {
    let user = await User.findOne({
      'email':'Ardith12@yahoo.com',
      'isActive':true,
      'isDeleted':false
    });
    let userRole = await Role.findOne({ code: 'SYSTEM_USER' }, { id: 1 });
    if (user && user.isPasswordMatch('sgtNNPQ7bW8CxjQ') && userRole){
    ...
    let admin = await User.findOne({
      'email':'Jovany_Little17@yahoo.com',
      'isActive':true,
      'isDeleted':false
    });
    let adminRole = await Role.findOne({ code: 'SYSTEM_USER' }, { id: 1 });
    if (admin && admin.isPasswordMatch('9VD2vxeooRWfSKy') && adminRole){
      let count = await UserRole.countDocuments({
        userId: admin.id,
        roleId: adminRole.id,
        isActive: true, 
...
async function seedUser () {
  try {
    ...
    if (!user || !user.isPasswordMatch('sgtNNPQ7bW8CxjQ') ) {
      let user = new User({
    ...
    if (!admin || !admin.isPasswordMatch('9VD2vxeooRWfSKy') ) {
      let admin = new User({
