...
async function seedUser () {
  try {
    ...
    userToBeInserted = await dbService.findOne(model.user,{ 'username':'Tiffany10' });
    if (!userToBeInserted) {  
      userToBeInserted = {
      ...
      await dbService.updateByQuery(model.user, { 'username':'Tiffany10' }, userToBeInserted);
    }
    userToBeInserted = await dbService.findOne(model.user,{ 'username':'Lucie91' });
    if (!userToBeInserted) {  
      userToBeInserted = {
      ...
      userToBeInserted.password = await bcrypt.hash(userToBeInserted.password, 8);
      await dbService.updateByQuery(model.user, { 'username':'Lucie91' }, userToBeInserted);
    }
