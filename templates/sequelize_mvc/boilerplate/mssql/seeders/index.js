...
async function seedUser () {
  try {
    ...
    userToBeInserted = await dbService.findOne(model.user,{ 'username':'Velma.Abbott' });
    if (!userToBeInserted) {  
      userToBeInserted = {
        'password':'YMu1jWWvDIilj8x',
        'username':'Velma.Abbott',
        'isDeleted':false,
        'isActive':true,
      ...
      await dbService.updateByQuery(model.user, { 'username':'Velma.Abbott' }, userToBeInserted);
    }
    userToBeInserted = await dbService.findOne(model.user,{ 'username':'Cornell31' });
    if (!userToBeInserted) {  
      userToBeInserted = {
        'password':'vz64aeaoMVGdlXT',
        'username':'Cornell31',
        'isDeleted':false,
        'isActive':true,
      ...
      userToBeInserted.password = await bcrypt.hash(userToBeInserted.password, 8);
      await dbService.updateByQuery(model.user, { 'username':'Cornell31' }, userToBeInserted);
    }
