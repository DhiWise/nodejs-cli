...
async function seedUserRole (){
  try {
    let user = await dbService.findOne(model.user,{
      'email':'Terence.McGlynn@gmail.com',
      'isActive':true,
      'isDeleted':false
    });
    ...
    if (user && user.isPasswordMatch('8LboeRugCQAEItE') && userRole){
    ...
    let admin = await dbService.findOne(model.user,{
      'email':'Breanna.Spinka@yahoo.com',
      'isActive':true,
      'isDeleted':false
    });
    ...
    if (admin && admin.isPasswordMatch('YLz2x_AOxkh4ihk') && adminRole){
      let count = await dbService.count(model.userRole, {
        userId: admin.id,
        roleId: adminRole.id,
        isActive: true, 
...
async function seedUser () {
  try {
    ...
    if (!user || !user.isPasswordMatch('8LboeRugCQAEItE') ) {
      let user = {
    ...
    if (!admin || !admin.isPasswordMatch('YLz2x_AOxkh4ihk') ) {
      let admin = {
